...
export default function AIRobotCompanion({ style, className = '' }) {
  ...
  // 2. Global Click & Tap Wink Detector (Priority 3 — Blocked during Celebration or Scroll)
  useEffect(() => {
    let winkTimeoutId;
    const handlePointerDown = () => {
      // IF CELEBRATING OR SCROLLING: STRICTLY SKIP WINK ANIMATION!
      if (isCelebratingRef.current || isScrolling) return;

      isWinkingRef.current = true;
      setIsWinking(true);
      targetHeadRef.current = { rx: -5, ry: 3, rz: 4 };

      clearTimeout(winkTimeoutId);
      winkTimeoutId = setTimeout(() => {
        setIsWinking(false);
        isWinkingRef.current = false; // UNLOCK after exact 240ms natural wink!
        if (!isCelebratingRef.current && !isScrolling) {
