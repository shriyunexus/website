/**
 * ShowcaseMockup — Continuous crossfade edition (1:1 with website - Copy)
 * ------------------------------------------------------------------------
...
import { getStageScale, STAGE_W as SHARED_STAGE_W, STAGE_H as SHARED_STAGE_H } from './stageScale';
...
export default function ShowcaseMockup({ scrollProgress: p = 0 }) {
   ...
   *
   *  PHASE 1 (p: 0.00 → 0.25) — CALM RISE TO EXACT CENTER
   *    Tablet glides smoothly into centered position over the first 25% of scroll.
   *    Scale = 1.0. Frame intact.
   *
   *  PHASE 2 (p: 0.25 → 1.00) — CONTROLLED FULL-SCREEN EXPANSION
   *    Uses a smooth ease-out curve so that right at this exact centered moment,
   *    size expansion begins smoothly and fills the viewport!
   *
   * ────────────────────────────────────────────────────────────────────── */
  const STAGE_W = SHARED_STAGE_W;
  const STAGE_H = SHARED_STAGE_H;
  const CARD_TOP = 738;
  const CARD_H = 504;
  const CARD_W = 1000;
  ...
  // Same scale HeroStage actually applies to .stage — critical so the
  // full-screen expansion below fills the real viewport exactly, on every
  // breakpoint (including the mobile/tablet "floor" range).
  const stageScale = getStageScale(winW);
